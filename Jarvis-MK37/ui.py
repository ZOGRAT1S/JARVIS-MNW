...
class JarvisUI:
    def __init__(self, face_path, size=None):
        self.root = tk.Tk()
        self.root.title("J.A.R.V.I.S — MARK NEW WORLD")
    ...
    def _draw(self):
        ...
        if self._has_face:
            ...
            orb_r = int(self.FACE_SZ * 0.30 * self.scale)
        for i in range(8, 0, -1):
            ...
            if self.muted:
                ...
                col = self._ac(255, 150, 0, ga)
            else:
                col = self._ac(0, 100, 255, ga)
            c.create_oval(FCX-r2, FCY-r2, FCX+r2, FCY+r2, fill=col, outline="")

        sphere_r = int(self.FACE_SZ * 0.38)
        for i in range(120):
            angle = math.radians((i / 120) * 360 + self.tick * (1.5 if self.speaking else 0.6))
            wobble = math.sin(self.tick * 0.05 + i * 0.3) * (12 if self.speaking else 4)
            r = sphere_r + int(wobble)
            px = FCX + int(r * math.cos(angle))
            py = FCY + int(r * math.sin(angle) * 0.55)
            size = 3 if self.speaking else 2
            if self.muted:
                col = self._ac(255, 30, 80, int(self.halo_a * 1.5))
            elif self.speaking:
                col = self._ac(255, 200, 0, int(self.halo_a * 1.5))
            else:
                col = self._ac(0, 212, 255, int(self.halo_a * 1.2))
        ...
        c.create_text(W // 2, H - 14, fill=C_DIM, font=("Courier", 8),
                      text="AZIZOV.SH  ·  CLASSIFIED  ·  MARK NEW WORLD")
